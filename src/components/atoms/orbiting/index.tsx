...
const OrbitingSkill = memo(({ config, angle }: OrbitingSkillProps) => {
  ...
  return (
    ...
    >
      <div
        className={`relative flex h-full w-full cursor-pointer items-center justify-center rounded-full bg-gray-800/90 p-2 backdrop-blur-sm transition-all duration-300 ${isHovered ? 'scale-125 shadow-2xl' : 'shadow-lg hover:shadow-xl'} `}
        style={{
          boxShadow: isHovered
      ...
      >
        ...
        {isHovered && (
          <div className='pointer-events-none absolute -bottom-8 left-1/2 -translate-x-1/2 rounded bg-gray-900/95 px-2 py-1 text-xs whitespace-nowrap text-white backdrop-blur-sm'>
            {label}
          </div>
        )}
      </div>
